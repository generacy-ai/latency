...
**Answer**: A — Add missing lint script. One-line consistency fix; using `--if-present` would mask genuinely missing scripts in the future.
...
**Answer**: A — Convert to `workspace:^`. Changesets will automatically resolve these on publish; fixed ranges will fall out of sync as packages version up.
...
**Answer**: A — Keep lint step. The `lint` script is a slot that will eventually hold a real linter; keeping it in CI means the workflow won't need to change when that swap happens. The 30-60s overhead is negligible.
...
**Answer**: A — Accept default format. The spec format was illustrative, not prescriptive. What matters is the `@preview` dist-tag, clear pre-release identification, and chronological sorting — all satisfied by the default format.
...
**Answer**: A — Keep both triggers as spec'd. The spec explicitly says CI runs on PRs to any branch. The push trigger validates the merged result. Duplicate run cost (~2-3 min) is minimal for the confidence gained.
...
**Answer**: A — Standardize on `NODE_AUTH_TOKEN`. `actions/setup-node` with `registry-url` generates an `.npmrc` referencing `NODE_AUTH_TOKEN`. The `changesets/action` also works with it. Consistent naming avoids confusion.
...
**Answer**: A — Add placeholder test scripts. Same reasoning as Q1: consistency and explicitness. The echo output makes it obvious which packages need real tests. Using `--if-present` would silently skip packages, hiding the gap.
...
**Answer**: A — Document only. Branch protection is a one-time manual setup that requires CI workflows to exist on `main` first. The PR description should include the exact `gh api` commands to run after merging to `develop` and syncing `main`.
...
**Answer**: A — No grouping. At this stage (all packages at 0.1.0, no external consumers), the complexity of linked/fixed groups isn't justified. Interface changes will naturally accompany plugin changes in the same PR. Can be reconsidered when version drift becomes a real problem.
...
**Answer**: B — Chain with `workflow_run`. Publishing preview packages that failed CI is counterproductive. Using `workflow_run` ensures preview publish only fires after CI succeeds, avoids duplicate install/build, and keeps workflows focused. The added latency is acceptable for preview publishes.
