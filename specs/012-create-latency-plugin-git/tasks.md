...
- [ ] T001 Create `packages/git-interface/` directory structure with `package.json`, `tsconfig.json`, `vitest.config.ts`
...
- [ ] T002 Implement `packages/git-interface/src/types.ts` — define `GitCommit`, `GitBranch`, `GitBlame`, `GitConfig` interfaces
...
- [ ] T003 [P] Implement `packages/git-interface/src/guards.ts` — `isGitCommit`, `isGitBranch` type guards
  - `isGitCommit`: check for `shortSha` and `tree` properties
  - `isGitBranch`: check for `ahead` and `behind` properties
- [ ] T004 [P] Implement `packages/git-interface/src/helpers.ts` — `formatCommitMessage`, `formatShortSha` utilities
- [ ] T005 Implement `packages/git-interface/src/index.ts` — re-export all public API from types, guards, helpers

## Phase 2: git-interface Tests

- [ ] T006 [P] Write `packages/git-interface/__tests__/guards.test.ts` — test type guards with valid/invalid inputs
...
- [ ] T007 [P] Write `packages/git-interface/__tests__/helpers.test.ts` — test `formatCommitMessage` and `formatShortSha`
- [ ] T008 Verify `git-interface` package builds and tests pass (`pnpm build && pnpm test` from package dir)

## Phase 3: latency-plugin-git Package Setup

- [ ] T009 Create `packages/latency-plugin-git/` directory structure with `package.json`, `tsconfig.json`, `vitest.config.ts`
...
- [ ] T010 Implement `packages/latency-plugin-git/src/errors.ts` — `mapGitError` function mapping git errors to `FacetError`
...
- [ ] T011 Implement `packages/latency-plugin-git/src/client.ts` — `GitClient` wrapper class around `simple-git`
...
- [ ] T012 Implement `packages/latency-plugin-git/src/mappers.ts` — mapper functions from simple-git types to domain types
...
- [ ] T013 Implement `packages/latency-plugin-git/src/plugin.ts` — `GitPlugin` class extending `AbstractSourceControlPlugin`
...
- [ ] T014 Implement `packages/latency-plugin-git/src/index.ts` — public exports (GitPlugin, GitConfig, re-exports from git-interface)

## Phase 5: latency-plugin-git Tests

- [ ] T015 [P] Write `packages/latency-plugin-git/__tests__/errors.test.ts` — test error pattern matching
  - Test each git error pattern maps to correct FacetError code
  - Test unknown errors map to UNKNOWN
- [ ] T016 [P] Write `packages/latency-plugin-git/__tests__/mappers.test.ts` — test all mapper functions
...
- [ ] T017 Write `packages/latency-plugin-git/__tests__/plugin.test.ts` — test GitPlugin with mocked GitClient
...
- [ ] T018 Verify `latency-plugin-git` package builds and tests pass

## Phase 6: Integration Verification

- [ ] T019 Run full monorepo build (`pnpm build` from root) and fix any issues
- [ ] T020 Run full test suite (`pnpm test` from root) and fix any failures
